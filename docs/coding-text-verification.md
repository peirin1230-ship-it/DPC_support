...
- `D.sd[cls][sdVal] = { a: [...], n: [...], s: [...] }`: 副傷病の対象ICDコードを副傷病フラグ別に保持（a=フラグ1 共通、n=フラグ2 手術なしの場合、s=フラグ3 手術ありの場合）
- `getSubdiagICDs(cls, sdVal, surgVal)`: DPCの手術区分（99=手術なし）に応じて該当するICDのみを返す
- データ調査結果（令和8年度）: フラグ1 583行、フラグ2 283行、フラグ3 510行。電子点数表の「対応コード」（副傷病値）自体は全て "1"

**判定: 適合** — 2026-09-02 の更新で副傷病フラグを反映。従来（2026-02-11 時点）は「D.sdにはフラグ1のみ」と記載していたが、これは電子点数表の「対応コード」列（副傷病値）を見た結果であり、別列の「副傷病フラグ」（1/2/3）は未反映だった。現在は詳細モーダル・サジェストの副傷病ステップで手術あり／なしに応じた対象ICDを表示する。
