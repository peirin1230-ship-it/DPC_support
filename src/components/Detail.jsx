...
// Row / Sec はコンポーネント外で定義する（レンダー内で定義すると再レンダー毎に再マウントされ、開閉状態が失われる）
const Row = ({ l, v, c }) => (
  <div style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: "1px solid #E0E0E0" }}>
    <span style={{ color: "#737373", fontSize: 14 }}>{l}</span>
    <span style={{ color: c || "#404040", fontSize: 14, fontWeight: 500, textAlign: "right", maxWidth: "60%" }}>{v}</span>
  </div>
);
...
function Sec({ title, children }) {
  ...
  return (
    <div style={{ marginTop: 8 }}>
      <button onClick={() => setO(!o)} aria-expanded={o} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", color: "#737373", cursor: "pointer", fontSize: 13, fontWeight: 600, padding: 0 }}>
        <span style={{ transform: o ? "rotate(90deg)" : "none", transition: "transform .15s", display: "inline-block" }}>▸</span>{title}
      </button>
      {o && <div style={{ marginTop: 6, background: "#FAFAFA", borderRadius: 6, padding: 8, maxHeight: 180, overflow: "auto" }}>{children}</div>}
    </div>
  );
}

const SCOPE_LABEL = { n: "手術なしの場合", s: "手術ありの場合" };
const codeWithName = (c, max = 15) => `${c}(${(D.cn[c] || "").slice(0, max)})`;
const sortSurgVals = (a, b) => {
  if (a[0] === "99") return 1; if (b[0] === "99") return -1;
  if (a[0] === "97") return 1; if (b[0] === "97") return -1;
...
export default function Detail({ r, onClose, sd, onSearchCls, isMobile }) {
  ...
  if (!r) return null;
  const cls = r.cls;
  const icds = D.icd[cls] || [];
  const si = D.si[cls] || {};
  const p1e = D.p1[cls] || {};
  const p2e = D.p2[cls] || {};
  const pc = D.pc?.[cls] || {};
  const tot = calcTotal(r.days, r.points, sd);
  const svDef = D.sv?.[cls];
  const validSv = classSurgVals(cls);
  const surgRows = Object.entries(si).filter(([c]) => validSv.has(c)).sort(sortSurgVals);
  const sdICDs = r.hasSdBranch && r.sdVal !== "0" ? getSubdiagICDs(cls, r.sdVal, r.surgVal) : [];
  const svEntries = svDef ? Object.entries(svDef).filter(([k]) => k !== "name").sort((a, b) => parseInt(a[0]) - parseInt(b[0])) : [];
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.3)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: isMobile ? 0 : 20 }} onClick={onClose}>
      <div ref={modalRef} role="dialog" aria-modal="true" aria-label={`DPC詳細 ${r.code}`} style={{ background: "#FFFFFF", borderRadius: isMobile ? 0 : 12, border: isMobile ? "none" : "1px solid #E0E0E0", boxShadow: isMobile ? "none" : "0 16px 48px rgba(0,0,0,.12)", maxWidth: isMobile ? "100vw" : "90vw", width: isMobile ? "100%" : 520, maxHeight: isMobile ? "100vh" : "85vh", height: isMobile ? "100vh" : "auto", overflow: "auto", padding: 0 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", gap: 0 }}>
              {["Ⅰ", "Ⅱ", "Ⅲ"].map((l, i) => (
                <div key={i} style={{ flex: 1, textAlign: "center", padding: "10px 0", background: i === 0 ? "rgba(59,130,246,.06)" : "transparent", borderRadius: i === 0 ? 8 : 0 }}>
                  <div style={{ color: "#737373", fontSize: 12 }}>期間{l}（{r.points[i] ? `${r.days[i]}日` : "-"}）</div>
                  <div style={{ fontFamily: M, fontWeight: 700, color: i === 0 ? "#3B82F6" : "#737373", fontSize: i === 0 ? 26 : 18 }}>{r.points[i] ? r.points[i].toLocaleString() : "-"}</div>
                  <div style={{ color: "#737373", fontSize: 11 }}>点/日</div>
                </div>
              ))}
            </div>
            {tot && (
              <div style={{ background: "#FAFAFA", borderRadius: 8, padding: 12, textAlign: "center", marginTop: 10 }}>
        ...
        <div style={{ padding: "12px 20px" }}>
          <Row l="手術" v={r.hasSurgBranch === false ? "（手術による分岐なし）" : (r.surgeryName || "なし")} c={r.hasSurgBranch === false ? "#737373" : undefined} />
          ...
          {sdICDs.length > 0 && (
            <div style={{ background: "#FAFAFA", borderRadius: 6, padding: 8, marginBottom: 4, maxHeight: 120, overflowY: "auto" }}>
              <div style={{ fontSize: 11, color: "#737373", fontWeight: 600, marginBottom: 4 }}>
              ...
              {sdICDs.map((ic, i) => (
                <div key={i} style={{ fontSize: 12, color: "#737373", padding: "1px 0", display: "flex", gap: 6 }}>
                  ...
                  <span>{ic.name || ""}</span>
                  {ic.scope !== "a" && !r.hasSurgBranch && <span style={{ fontSize: 10, color: "#A3A3A3" }}>（{SCOPE_LABEL[ic.scope]}）</span>}
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{ padding: "0 20px 12px" }}>
            ...
            <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
              {svEntries.map(([v, lb]) => {
                ...
                return (
                  <div key={v} style={{ background: cur ? "rgba(245,158,11,.08)" : "#FAFAFA", border: cur ? "1px solid #F59E0B" : "1px solid #E0E0E0", borderRadius: 4, padding: "4px 8px", fontSize: 12 }}>
        ...
        <div style={{ padding: "0 20px 16px" }}>
          <Sec title={`対象ICD-10（${icds.length}件）`}>
            {icds.slice(0, 100).map((c, i) => (
              <div key={i} style={{ fontSize: 12, color: "#737373", padding: "2px 0", display: "flex", gap: 6 }}>
                <span style={{ color: "#3B82F6", fontFamily: M, flexShrink: 0, minWidth: 48, fontSize: 11 }}>{c}</span>
                <span>{cleanName(D.icn[c] || "")}</span>
              </div>
            ))}
            {icds.length > 100 && <div style={{ color: "#737373", fontSize: 11, marginTop: 2 }}>他{icds.length - 100}件</div>}
          </Sec>
          {r.hasSurgBranch !== false && surgRows.length > 0 && (
            <Sec title={`手術定義（${surgRows.length}区分）`}>
              {surgRows.map(([corr, idx], i) => (
                <div key={i} style={{ color: "#404040", marginBottom: 3, fontSize: 12 }}>
                  ...
                  {(D.sl[idx] || []).slice(0, 3).map(k => codeWithName(k)).join(", ")}
                  {(D.sl[idx] || []).length > 3 && <span style={{ color: "#A3A3A3" }}> 他{D.sl[idx].length - 3}件</span>}
                </div>
              ))}
            </Sec>
          )}
          {r.hasP1Branch && Object.keys(p1e).length > 0 && (
            <Sec title={`処置等１定義（${Object.keys(p1e).length}区分）`}>
              {Object.entries(p1e).sort((a, b) => parseInt(b[0] || 0, 36) - parseInt(a[0] || 0, 36)).map(([corr, codes], i) => {
                ...
                return (
                  <div key={i} style={{ color: "#404040", marginBottom: 3, fontSize: 12 }}>
                    ...
                    {conds.map(c => (
                      <div key={c} style={{ fontSize: 11, color: "#A16207", marginLeft: 8 }}>※ {c} は {pc[c].slice(0, 3).join(", ")}{pc[c].length > 3 ? " 他" : ""} との組み合わせ時に評価</div>
                    ))}
                  </div>
                );
              })}
