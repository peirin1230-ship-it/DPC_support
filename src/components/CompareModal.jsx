...
export default function CompareModal({items,onClose,sd}){
  ...
  const tots=ti.map(t=>t?t.total:0);const maxT=Math.max(...tots);
  ...
  const fields=[
    ...
    ["期間Ⅰ",r=>r.days[0]?r.days[0]+"日":"-",1],["期間Ⅱ",r=>r.days[1]?r.days[1]+"日":"-",1],["期間Ⅲ",r=>r.days[2]?r.days[2]+"日":"-",1],
    ["点数Ⅰ/日",r=>r.points[0]||"-",1],["点数Ⅱ/日",r=>r.points[1]||"-",1],["点数Ⅲ/日",r=>r.points[2]||"-",1],
  ];
  return(
          ...
          <tbody>
            {fields.map(([lb,fn,mono],fi)=>{
              ...
              return(<tr key={fi}>
                ...
                {items.map((it,ii)=>{const v=fn(it);const dk=lb==="区分"&&v==="出来高";const diff=!same&&fi>0;
                  return(<td key={ii} style={{padding:"6px 10px",borderBottom:"1px solid #1e293b",color:dk?"#ef4444":diff?"#fbbf24":"#cbd5e1",fontWeight:dk||diff?600:400,fontFamily:mono?M:"inherit"}}>{v}</td>);
                })}
              </tr>);
            })}
            {sd>0&&(<>
              <tr><td style={{padding:"8px 10px",background:"#0f172a",color:"#38bdf8",fontWeight:700,borderBottom:"1px solid #1e293b",fontSize:13}}>総点数</td>
                {items.map((it,ii)=>{const t=tots[ii];const best=t===maxT&&t>0;return(<td key={ii} style={{padding:"8px 10px",borderBottom:"1px solid #1e293b",color:it.isDekidaka?"#ef4444":best?"#38bdf8":"#cbd5e1",fontWeight:700,fontFamily:M,fontSize:16,background:best?"rgba(56,189,248,.06)":"#0f172a"}}>{it.isDekidaka?"出来高":(t?t.toLocaleString():"-")}</td>);})}</tr>
            </>)}
          </tbody>
        </table>
      </div>
